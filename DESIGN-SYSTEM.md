...
- **AI 아바타 인터랙션(전역 통일)**: 아바타는 `.ai-avatar.gen`에 애니메이션(`.ai-ind`)+정적 로고(`.ai-hex`)를 겹쳐두고, 생성 완료 시 `.settled` 클래스로 **리니어 스무스 크로스페이드**(opacity 520ms linear). **`...중`(process) 메시지는 결과가 나올 때까지 계속 회전**(하단 process 스피너와 싱크) — settle 안 함. 결과 메시지만 reveal 후 ~0.75s에 settle. 정적 로고(`.ai-hex`)는 인디케이터보다 10% 작게(`scale(0.9)`).
- **파일 칩 등장**: `role:'file'` 칩은 살짝 위에서 떨어져 자리잡는 드래그&드롭 업로드 모션(`fileDrop` + `fileDropRing`).
- **아티펙트 패널 등장(전역 통일)**: `scene.open` 패널은 항상 `artifact-body.scrollTop=0`로 **최상단부터** 보이고, 잠시 멈춘 뒤 위→아래로 훑음(`skimArtifact`). 내용이 길어 스크롤이 길어지면 **일정 지점까지만(SPEED·MAX_MS 상한)** 내려가고, 끝나면 **바로 다음 액션으로 자연스럽게 전환**(끝에서 멈춰있지 않음).
