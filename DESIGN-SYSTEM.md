...
- **AI 아바타 인터랙션(전역 통일)**: 모든 AI 메시지 아바타는 생성 중 소프트 헥사곤 애니메이션(`STAR`/`.ai-ind`)으로 돌다가, 생성(reveal) 완료 시 솔리드 스파클 로고(`STATIC_HEX`/`.ai-hex`)로 멈춤(`settleAvatar`). 생성중 상태는 reveal 후 ~0.75s 유지.
